...
async fn main() {
    ...
    let coupons = coupon_client.edit("6488d83b966d4", CouponCreatePayload {
        code: "test_coupon",
        discount_value: 3,
        max_uses: None,
        products_bound: None,
    ...
    if let Err(err) = coupons {
        println!("error: unable to edit coupons - {}", err);
    } else if let Ok(success) = coupons {
        println!("updated coupon: {}", success);
    }
}