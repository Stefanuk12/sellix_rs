...
async fn main() {
    ...
    let whitelists = whitelist_client.delete("648846c96b386").await;
    if let Err(err) = whitelists {
        println!("error: unable to remove whitelist - {}", err);
    } else if let Ok(all) = whitelists {
        println!("whitelist removed: {}", all);
    }
}