...
async fn main() {
    ...
    let whitelists = whitelist_client.create(WhitelistCreatePayload {
        r#type: WhitelistTypes::Email,
        data: "test@example.com",
        note: Some("Testing")
    }).await;
    if let Err(err) = whitelists {
        println!("error: unable to create whitelists - {}", err);
    } else if let Ok(created) = whitelists {
        println!("created whitelist with uniqid {}", created.uniqid);
    }
}